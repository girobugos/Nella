...
use Nette\Forms\MultiSelectBox;
use Nette\Forms\RadioList;
 ...
 * @package    Nella
 */
final class FormClientScript extends Nette\Object
{
	...
	public $doAlert = 'if (element) element.focus(); element.after(\'<p class="error">\'+message+\'</p>\');';
	...
	private function getValidateScript(Rules $rules, $onlyCheck = FALSE)
	{
		...
		foreach ($rules as $rule) {
			...
			if (!empty($rule->message)) { // this is rule
				if ($onlyCheck) {
					$res .= "$script\n\tif (" . ($rule->isNegative ? '' : '!') . "res) { valid = false; }\n\t";
	...
	private function getValueScript(IFormControl $control)
	{
		...
		$tmp = "element = \$('#" . $control->getHtmlId() . "');\n\t";
		switch (TRUE) {
		case $control instanceof Checkbox:
			return $tmp . "var val = element.is(':checked');\n\t";

		case $control instanceof RadioList:
			return "for (var val=null, i=0; i<" . count($control->getItems()) . "; i++) {\n\t\t"
			. "element = \$('#" . $control->getHtmlId() . '-' . "+i');\n\t\t"
			. "if (element.is(':checked')) { val = element.val(); break; }\n\t"
			. "}\n\t";

		default:
			return $tmp . "var val = element.val().replace(/^\\s+|\\s+\$/g, '');\n\t";
		}
	}
