...
use Nette\Forms\INamingContainer;
 ...
 * @version    $Id$
 */
class DataGrid extends Nette\Application\Control implements ArrayAccess, INamingContainer
{
	/** @persistent int */
	public $page = 1;
	 ...
	 */
	public function addAction($title, $signal, $icon = NULL, $ajaxClass = NULL, $key = DataGridAction::WITH_KEY)
	{
		if (!$this->hasColumns('ActionColumn')) {
			throw new InvalidStateException('No ActionColumn defined. Use DataGrid::addActionColumn before you add actions.');
		}

		return $this->currentActionColumn->addAction($title, $signal, $icon, $ajaxClass, $key);
	}
