...
class DataGridRenderer extends Nette\Object implements IDataGridRenderer
{
	...
	public function renderPaginator()
	{
		...
		// to-first button
		$first = $this->getWrapper('paginator button first')->setHtml("&#x25C4;");
		...
		// previous button
		$prev = $this->getWrapper('paginator button prev')->setHtml("&lt;");
		...
		// next button
		$next = $this->getWrapper('paginator button next')->setHtml("&gt;");
		...
		// to-last button
		$last = $this->getWrapper('paginator button last')->setHtml("&#x25BA;");
		...
		foreach ($this->dataGrid->getColumns() as $column) {
			...
			if ($column instanceof ActionColumn) {
				...
				foreach ($this->dataGrid->getActions() as $action) {
					if (!is_callable($action->ifCallback) || call_user_func($action->ifCallback, $data)) {
						$html = $action->getHtml();
